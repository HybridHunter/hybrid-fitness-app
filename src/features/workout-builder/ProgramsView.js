...
export default function ProgramsView({programs,setPrograms,workouts}){
  ...
  const printProgram=(prog,pw)=>{if(pw.length)printWorkoutSheet(pw,{title:prog.name});};
    ...
    <div style={{display:"grid",gap:16}}>{programs.map(prog=>{const pw=workouts.filter(w=>prog.workoutIds.includes(w.id));const avail=workouts.filter(w=>!prog.workoutIds.includes(w.id));return(
      <div key={prog.id} style={{background:B.card,borderRadius:12,border:"1px solid "+B.border,padding:20}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          ...
          <button onClick={()=>printProgram(prog,pw)} style={{background:B.accent+"15",border:"1px solid "+B.accent+"30",borderRadius:6,color:B.accent,cursor:"pointer",fontSize:10,padding:"5px 12px",fontWeight:700,marginRight:6}}>Print Program</button><button onClick={()=>setConfirm({msg:"Delete this program? This cannot be undone.",action:()=>{setPrograms(p=>p.filter(x=>x.id!==prog.id));setConfirm(null);}})} style={{background:B.red+"15",border:"1px solid "+B.red+"30",borderRadius:6,color:B.red,cursor:"pointer",fontSize:10,padding:"5px 12px",fontWeight:700}}>Delete</button>
        </div>
